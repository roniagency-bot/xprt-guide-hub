...
const BOND_TYPES = [
  ...
  "Notary Bond",
  "Fidelity Bond",
  "Document Preparation / LDA Bond",
  "Other / Not Sure",
];

export function BondsLeadForm({
  leadMagnetId,
  leadMagnetSlug,
  resourceName,
  thankYouSlug,
  ...
  const [stateVal, setStateVal] = useState<"NV" | "CO" | "OTHER" | "">("");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!bondType) {
      setErrors({ bond_type: "Please select a bond type" });
      return;
    }
    ...
    const payload = {
      ...
      category_tag: "bonds",
      source_path,
      // Only NV/CO are accepted by the server schema; "OTHER" is sent only via meta.
      state: stateVal === "NV" || stateVal === "CO" ? stateVal : undefined,
      meta: {
        first_name,
        last_name,
        ...
        bond_type: bondType,
        state_label: stateVal === "OTHER" ? "Other" : stateVal,
      },
    };
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select value={stateVal} onValueChange={(v) => setStateVal(v as never)}>
        ...
        <div className="space-y-2">
          <Label htmlFor={`${leadMagnetSlug}-bond_type`}>Bond type</Label>
          <Select value={bondType} onValueChange={setBondType}>
            <SelectTrigger id={`${leadMagnetSlug}-bond_type`}>
          ...
          {errors.bond_type && (
            <p className="text-sm text-destructive">{errors.bond_type}</p>
          )}
        </div>
      </div>
